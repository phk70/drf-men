...
Для постраничного разбиения данных в библиотеке REST FRAMEWORK в settings добавим строки
...

{
    ...
    "results": [
        ...
        },
        {
            ...
            "content": "Американский актёр.\r\n\r\nДжилленхол с раннего детства был связан с миром кино, начал сниматься уже в 11 лет. Свою первую роль он сыграл в фильме «Городские пижоны» (1991), после чего последовали уже небольшие роли в различных фильмах. Мирового признания Джейкоб Джилленхол добился в 1999 году, когда на экраны вышла биографическая драма «Октябрьское небо», где он исполнил свою первую главную роль — подростка Хомера Хикэма. Также он исполнил главную роль в картине «Донни Дарко». С этого момента молодому актёру начинают предлагать роли в различных проектах: он играет школьника, попавшего в глобальный катаклизм в фильме-катастрофе Роланда Эммериха «Послезавтра», морского пехотинца в фильме «Морпехи» Сэма Мендеса; в 2005 году исполнил роль Джека Твиста в высоко оценённом кинокритиками фильме Энга Ли «Горбатая гора» и был удостоен премии BAFTA и единственной в карьере номинации на «Оскар». В дальнейшем получил высокую похвалу за работу в фильмах «Зодиак», «Патруль» «Пленницы», «Стрингер» и «Под покровом ночи».\r\n\r\nДжилленхол является трехкратным номинантом награды Британской киноакадемии, двукратным номинантом на «Золотой глобус» и Премию Американской гильдии киноактёров, а также трехкратным номинантом на театральную премию «Тони».",
            "time_create": "2024-12-08T12:43:03.590700+07:00",
            "time_update": "2024-12-08T12:43:03.590700+07:00",
            "is_published": true,
            "cat": 1
        }
    ]
...
Но эта пагинация устанавливается глобально для любого апи запроса и всех элементов списка.

Если для какого либо апи нужны другие настройки пагинации, то она настраивается во views
Для этого создается свой класс пагинации.
На примере MenAPIList:
...
Теперь по запросу на http://127.0.0.1:8000/api/v1/men/ список мужчин будем выдаваться по 4 записи, хотя в глобальных настройках установлено 2.
Но для других маршрутов, которые не относятся к классу MenAPIList будет действовать глобальные настройки.

page_size_query_param = 'page_size'  это параметр для гет запроса. Клиент может сам его прописать в запросе и указать сколько именно записей он хочет.
Например http://127.0.0.1:8000/api/v1/men/?page_size=1 и тогда быдет выводиться по 1 записи. Но оно не доблжно быть более max_page_size
У нас max_page_size=100 и даже если мы поставим в гет запросе 1000, то все равно будет выводиться по 100.